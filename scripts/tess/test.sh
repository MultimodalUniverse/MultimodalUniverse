...
if python -c "from datasets import load_dataset; dset = load_dataset('./tess.py', trust_remote_code=True, split='train').with_format('numpy'); print(f'loaded dataset with {len(dset)} examples'); next(iter(dset));"; then
