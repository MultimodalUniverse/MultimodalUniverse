...
class TGLC_Downloader:
    ...
    def processing_fn(
        ...
        fits_path = f'./{self.sector_str}/cam{row["cam"]}-ccd{row["ccd"]}/{row["fp1"]}/{row["fp2"]}/{row["fp3"]}/{row["fp4"]}/hlsp_tglc_tess_ffi_gaiaid-{row["gaiadr3_id"]}-s0023-cam{row["cam"]}-ccd{row["cam"]}_tess_v1_llc.fits'
    ...
    def save_in_standard_format(self, args):
        ...
        Parameters
        ----------
        args: tuple, tuple of arguments: (catalog, output_filename)
