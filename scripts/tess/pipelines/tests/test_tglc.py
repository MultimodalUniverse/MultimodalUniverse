...
def test_convert_fits_to_standard_format(create_sector_catalog):
    ...
    with h5py.File(test_hdf5_fp, 'r') as f:
        keys_ = list(f.keys())
        assert keys_.sort(key=len) == ['DEC', 'RA', 'TIC_ID', 'aper_flux', 'aper_flux_err', 'GAIADR3_ID', 'psf_flux', 'psf_flux_err', 'tess_flags', 'tglc_flags', 'time'].sort(key=len), f"Expected keys to be ['lightcurve', 'RA', 'DEC', 'TIC_ID', 'GAIADR3_ID', 'psf_flux_err', 'aper_flux_err'] in {test_hdf5_fp}. Got {keys_}."
