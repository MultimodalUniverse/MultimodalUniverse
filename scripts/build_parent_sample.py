...
def snana_bpf(file_dir, data, metadata, keys_data, keys_metadata, args):
    ...
    for file in files:
        ...
        metadata_, data_ = sncosmo.read_snana_ascii(
            os.path.join(file_dir, file), default_tablename="OBS"
        )
        data_ = data_["OBS"]
...
def main(args):
    ...
    if args.dataset not in SNANA_DATASETS:
        ...
        example_metadata, example_data = sncosmo.read_snana_ascii(
            os.path.join(file_dir, os.listdir(file_dir)[0]), default_tablename="OBS"
        )
    ...
    if args.dataset in SNANA_DATASETS:
        metadata["healpix"] = hp.ang2pix(
            16, metadata["RA"], metadata["DECL"], lonlat=True, nest=True
        )
    else:
        metadata["healpix"] = hp.ang2pix(
