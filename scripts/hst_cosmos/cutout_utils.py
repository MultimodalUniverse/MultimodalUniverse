...
def get_randoms_from_files(
    galaxy_df, fits_file_list, num_pix, max_randoms_per_file=None,
    col_name='fits_file'
):
    """
    Args:
        ...
        fite_file_list (list(str)): path for fits files to search through for
            matches with the catalog.
        num_pix (int): size in pixels of a cutout.
        max_randoms_per_file: maximum number of randoms to allow per fits file.
        column name to store list of fits files for each object in the catalog.

    Returns:
        (pd.DataFrame) dataframe with ra and decs for randoms.
    """

    # all the ra and decs for our galaxy catalog.
    galaxy_ra = galaxy_df['RA'].to_numpy()
    galaxy_dec = galaxy_df['DEC'].to_numpy()
    galaxy_coords = SkyCoord(ra=galaxy_ra * u.deg, dec=galaxy_dec * u.deg)

    randoms_df = {
        'RA': [], 'DEC': [], col_name: [], 'pixel_scale': [], 'MDRIZSKY': [],
        'CCDGAIN': [], 'AVG_READNSE': []
    }

    # go through the fits files and build a list of ra and decs within each
    # fits file.
    for fits_path in fits_file_list:
        ...
        # create a grid of random ra and decs. Avoid the edges and space out
        # by an extra 20 percent.
        x_grid, y_grid = np.meshgrid(
            np.arange(
                num_pix // 2, data_header['NAXIS1'] - num_pix // 2,
                int(num_pix * 1.2)
            ),
            np.arange(
                num_pix // 2, data_header['NAXIS2'] - num_pix // 2,
                int(num_pix * 1.2)
            )
        )

        # limit randoms.
        x_grid = x_grid.flatten()[:max_randoms_per_file]
        y_grid = y_grid.flatten()[:max_randoms_per_file]

        # sky coordinates for our grid.
        randoms_ra, randoms_dec = wcs.all_pix2world(x_grid, y_grid, 0)
        randoms_df['RA'].extend(randoms_ra)
        randoms_df['DEC'].extend(randoms_dec)
        randoms_df[col_name].extend([fits_path] * len(randoms_ra))
        randoms_df['pixel_scale'].extend([pixel_scale] * len(randoms_ra))
        randoms_df['MDRIZSKY'].extend([mdrizsky] * len(randoms_ra))
        randoms_df['CCDGAIN'].extend([ccd_gain] * len(randoms_ra))
        randoms_df['AVG_READNSE'].extend([avg_read_noise] * len(randoms_ra))

    # create pd.DataFrame
    randoms_df = pd.DataFrame(randoms_df)

    # reject any randoms that bump up against the galaxies in our catalog.
    random_coords = SkyCoord(
        ra=randoms_df['RA'].to_numpy() * u.deg,
        dec=randoms_df['DEC'].to_numpy() * u.deg
    )
    _, sep2d, _ = coordinates.match_coordinates_sky(
        random_coords, galaxy_coords
    )
    distance_mask = sep2d.arcsec / randoms_df['pixel_scale'] > num_pix

    return randoms_df[distance_mask]
