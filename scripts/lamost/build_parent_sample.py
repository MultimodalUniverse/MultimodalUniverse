    ...
    try:
        with fits.open(filename) as hdulist:
            ...
            if x.shape[1] > max_length:
                ...
                x = np.pad(x, ((0, 0), (0, max_length - x.shape[1])), mode='constant', constant_values=0)
            
            # Handle wavelength array the same way
            if wv.shape[1] > max_length:
                wv = wv[:, :max_length]
            elif wv.shape[1] < max_length:
                wv = np.pad(wv, ((0, 0), (0, max_length - wv.shape[1])), mode='constant', constant_values=0)
...
def save_in_standard_format(args):
    ...
    try:
        ...
        if len(catalog) != len(spectra):
            print(f"Warning: catalog has {len(catalog)} rows but spectra has {len(spectra)} rows")
            # Truncate catalog to match spectra if some failed
            catalog = catalog[:len(spectra)]
        
        catalog = hstack([catalog, spectra])
        print(f"Combined catalog and spectra: {len(catalog)} rows, {len(catalog.colnames)} columns")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "catalog_path",
        type=str,
        help="Path to LAMOST catalog file (FITS format)"
    )
    parser.add_argument(
        "lamost_data_path",
        type=str,
