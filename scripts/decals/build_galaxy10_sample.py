...
def run_download(df, save_dir, overwrite, n_processes=4):
    # download_image(train_df.iloc[0])

    pool = Pool(processes=n_processes)
    rows = [row for _, row in df.iterrows()]

    retry_wrapper_partial = partial(retry_wrapper, download_image, save_dir=save_dir, overwrite=overwrite)
...
def pack_to_hdf5(df: pd.DataFrame, save_dir: Path):
    df['file_loc'] = save_dir / df.apply(galaxy_to_filename, axis=1)
    # save to hdf5
    with h5py.File(save_dir / 'decals10.h5', 'w') as f:

        for col in ['ra', 'dec', 'label']:
            f.create_dataset(col, data=df[col].values)

        f.create_dataset('image_array', (3, 224, 224, len(df)))  # no values yet
...
def download_image(row, save_dir, overwrite=False):
    # download these specific images

    save_loc = save_dir / galaxy_to_filename(row)
...
def main():
    ...
    run_download(train_df, save_dir, overwrite, n_processes)
        
    pack_to_hdf5(train_df, save_dir)
