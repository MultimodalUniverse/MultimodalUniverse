...
def hdf5_to_parquet(hdf5_file, output_dir, nside):
    with h5py.File(hdf5_file, "r") as f:
        ...
        table = table.append_column(
            "healpix", [ang2pix(table["ra"], table["dec"], nside)]
        )
        table = table.append_column("object_id", table["source_id"])
...
def main(args):
    ...
    xp_files = sorted(
        glob.glob(os.path.join(args.input_dir, "XpContinuousMeanSpectrum*.hdf5"))
    )
    rvs_files = sorted(glob.glob(os.path.join(args.input_dir, "RvsMeanSpectrum*.hdf5")))
    ap_files = sorted(
        glob.glob(os.path.join(args.input_dir, "AstrophysicalParameters*.hdf5"))
    )
    ...
    process_map(
        partial(hdf5_to_parquet_partial, output_dir=source_dir),
        source_files,
        max_workers=args.num_workers,
        chunksize=1,
        desc="Building GaiaSource",
    )
    process_map(
        partial(hdf5_to_parquet_partial, output_dir=xp_dir),
        xp_files,
        max_workers=args.num_workers,
        chunksize=1,
        desc="Building XP",
    ...
    process_map(
        partial(hdf5_to_parquet_partial, output_dir=rvs_dir),
        rvs_files,
        max_workers=args.num_workers,
        chunksize=1,
        desc="Building RVS",
    ...
    process_map(
        partial(hdf5_to_parquet_partial, output_dir=ap_dir),
        ap_files,
        max_workers=args.num_workers,
        chunksize=1,
        desc="Building AP",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_workers", type=int, help="Number of workers", default=os.cpu_count()
    )
